...
-- Preflight must be ready before applying; approved assets require operator classification.
...
-- Existing published records stay published. Existing non-published records stay
-- non-published. Approved assets are not reclassified here; the SELECT-only
-- preflight fails closed when approved assets exist so operators can choose a
-- documented classification before this migration is applied.
...
create index if not exists assets_moderation_queue_idx on public.assets(status, submitted_at desc, creator_id) where status in ('pending_review', 'rejected', 'published', 'approved');
...
begin
  ...
  if not exists (
    ...
    where creators.id = current_asset.creator_id
      and creators.profile_id = auth.uid()
      and creators.active = true
      and creators.application_status = 'approved'
  ) then raise exception 'Only the approved owner creator can submit this asset'; end if;
...
begin
  ...
  if current_post.creator_id is null or not exists (
    select 1 from public.creators
    where creators.id = current_post.creator_id
      and creators.profile_id = auth.uid()
      and creators.active = true
      and creators.application_status = 'approved'
  ) then raise exception 'Only the approved owner creator can submit this blog post'; end if;
...
create or replace function public.set_asset_featured(target_asset_id uuid, target_featured boolean)
returns public.assets
language plpgsql
security definer
set search_path = public
as $$
declare updated_asset public.assets;
begin
  if not public.is_admin() then raise exception 'Admin access required'; end if;
  update public.assets set featured = target_featured where id = target_asset_id returning * into updated_asset;
  if not found then raise exception 'Asset not found'; end if;
...
revoke all on function public.set_asset_featured(uuid, boolean) from public;
...
revoke all on function public.set_asset_featured(uuid, boolean) from anon;
...
grant execute on function public.set_asset_featured(uuid, boolean) to authenticated;

-- Direct lifecycle writes are blocked; creators can edit only own draft/rejected
-- content columns. Trusted RPCs perform submission/review/admin-only transitions.
...
drop policy if exists "approved creators can create own editable assets" on public.assets;
...
drop policy if exists "approved creators can update own editable assets" on public.assets;
...
drop policy if exists "public and owners can read blog posts" on public.blog_posts;
...
drop policy if exists "approved creators can create own editable blog posts" on public.blog_posts;
