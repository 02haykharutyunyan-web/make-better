...
export function previewImport(kind: ImportKind, rows: ImportRow[]): ImportPreview {
  ...
  const valid = rows.filter(row => {
    ...
    if (kind === "asset" && !["external_url", "text"].includes(row.delivery_type)) { errors.push({ row: row.__row, message: "delivery_type must be external_url or text. Attach private files from the asset editor after import." }); return false; }
    return true;
  });
...
export async function runAdminImport(kind: ImportKind, rows: ImportRow[]) {
  ...
  return data as { ok: true; created: number; skipped: number; errors: { row: number; message: string }[] };
}
