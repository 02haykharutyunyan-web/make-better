...
export default function AdminBlog() {
  ...
  return (
    <AdminLayout eyebrow="Blog" title="Manage posts">
      ...
      {loading && <div className="mb-6 card-premium p-4 text-sm text-[#94A3B8]">Loading posts...</div>}
          ...
          <tbody>
            {posts.map(p => (
              <tr key={p.id || p.slug} className="border-t border-white/5">
                ...
                <td className="px-5 py-4 text-[#94A3B8]">{p.date}</td>
                <td className="px-5 py-4 text-right text-xs space-x-3">
                  <button onClick={() => setEditing({ ...p })} className="text-[#94A3B8] hover:text-white">Edit</button>
      ...
      {editing && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center p-3 sm:items-center sm:p-4">
          ...
          <form onSubmit={save} className="relative w-full max-w-2xl card-premium p-5 sm:p-7 space-y-4 max-h-[calc(100dvh-1.5rem)] overflow-y-auto">
            <h3 className="text-xl sm:text-2xl font-medium tracking-tight">{editing.slug ? "Edit post" : "New post"}</h3>
            ...
            <label className="block"><span className="text-xs text-[#94A3B8]">Author / Creator</span>
              <select value={editing.creatorSlug || ""} onChange={e => setEditing({ ...editing, creatorSlug: e.target.value || undefined })} className="mt-1 w-full rounded-xl bg-[#111827]/75 border border-[#1E293B] px-3.5 py-3 text-base sm:text-sm">
            ...
            <label className="block"><span className="text-xs text-[#94A3B8]">Status</span>
              <select value={editing.status || "Published"} onChange={e => setEditing({ ...editing, status: e.target.value as any })} className="mt-1 w-full rounded-xl bg-[#111827]/75 border border-[#1E293B] px-3.5 py-3 text-base sm:text-sm">
