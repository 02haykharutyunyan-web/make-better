...
export default function AdminCollections() {
  ...
  return (
    <AdminLayout eyebrow="Collections" title="Manage collections">
      ...
      {loading && <div className="mb-6 card-premium p-4 text-sm text-[#94A3B8]">Loading collections...</div>}

      <div className="grid gap-4 md:grid-cols-2">
        {collections.map(c => (
          <div key={c.id || c.slug} className="card-premium p-5 sm:p-6">
            <div className="flex flex-col items-start justify-between gap-4 sm:flex-row">
              <div><h3 className="text-lg font-medium tracking-tight">{c.title}</h3><p className="mt-1 text-sm text-[#94A3B8] line-clamp-2">{c.description}</p><div className="mt-2 text-xs text-[#94A3B8]/70">{c.status}</div></div>
              <div className="flex flex-wrap gap-2 text-xs">
                <button onClick={() => setEditing({ ...c })} className="rounded-full border border-[#1E293B] px-3 py-1.5 hover:bg-[#F97316]/10">Edit</button>
                <button onClick={() => remove(c)} className="rounded-full border border-[#1E293B] px-3 py-1.5 text-red-300 hover:bg-[#F97316]/10">Delete</button>
              </div>
            </div>
          </div>
        ))}
      ...
      {editing && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center p-3 sm:items-center sm:p-4">
          ...
          <form onSubmit={save} className="relative w-full max-w-2xl card-premium p-5 sm:p-7 space-y-4 max-h-[calc(100dvh-1.5rem)] overflow-y-auto">
            <h3 className="text-xl sm:text-2xl font-medium tracking-tight">{editing.slug ? "Edit collection" : "New collection"}</h3>
            ...
            <div><div className="text-xs text-[#94A3B8] mb-2">Selected published assets</div><div className="max-h-40 overflow-y-auto rounded-xl border border-[#1E293B] bg-[#0B1020]/70 p-3 space-y-1">
            ...
            <label className="block"><span className="text-xs text-[#94A3B8]">Status</span><select value={editing.status || "Published"} onChange={e => setEditing({ ...editing, status: e.target.value })} className="mt-1 w-full rounded-xl bg-[#111827]/75 border border-[#1E293B] px-3.5 py-3 text-base sm:text-sm"><option className="bg-black">Draft</option><option className="bg-black">Published</option></select></label>
            <div className="flex flex-col justify-end gap-3 pt-2 sm:flex-row"><button type="button" onClick={() => setEditing(null)} className="min-h-11 rounded-full border border-[#1E293B] px-4 py-2 text-sm">Cancel</button><button className="min-h-11 rounded-full btn-primary px-5 py-2 text-sm font-medium">Save</button></div>
          </form>
        </div>
      )}
    </AdminLayout>
...
function Field({ label, value, onChange, required, type = "text" }: any) {
  return <label className="block"><span className="text-xs text-[#94A3B8]">{label}{required && <span className="text-white/30"> *</span>}</span><input required={required} type={type} value={value} onChange={e => onChange(e.target.value)} className="mt-1 w-full rounded-xl bg-[#111827]/75 border border-[#1E293B] px-3.5 py-3 text-base sm:text-sm focus:outline-none focus:border-[#3B82F6]/70" /></label>;
}
function Textarea({ label, value, onChange, rows = 3 }: any) {
  return <label className="block"><span className="text-xs text-[#94A3B8]">{label}</span><textarea value={value} rows={rows} onChange={e => onChange(e.target.value)} className="mt-1 w-full rounded-xl bg-[#111827]/75 border border-[#1E293B] px-3.5 py-3 text-base sm:text-sm focus:outline-none focus:border-[#3B82F6]/70" /></label>;
}
