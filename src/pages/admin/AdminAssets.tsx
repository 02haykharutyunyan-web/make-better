      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {filters.map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`rounded-full px-3 py-1.5 text-xs border transition ${filter === f ? "bg-white text-black border-white" : "border-white/15 text-white/65 hover:text-white hover:bg-white/[0.06]"}`}>
            {f}
          </button>
        ))}
      </div>
      {err && <div className="mb-4 rounded-xl border border-red-400/20 bg-red-400/10 p-4 text-sm text-red-200">{err}</div>}
      {loading && <div className="mb-4 card-premium p-4 text-sm text-white/55">Loading assets...</div>}
                ...
                return (
                  <tr key={a.id} className="border-t border-white/5 align-top">
                    ...
                    <td className="px-5 py-4 text-white/65">
                      ...
                      {deliverables[a.id]?.file_name && <div className="text-[11px] text-white/40 max-w-[160px] truncate">{deliverables[a.id].file_name}</div>}
                      {deliverables[a.id]?.external_url && <div className="text-[11px] text-white/40 max-w-[160px] truncate">{deliverables[a.id].external_url}</div>}
                    </td>
                    <td className="px-5 py-4"><span className={`rounded-full border px-3 py-1 text-xs ${statusStyles[a.status]}`}>{a.status}</span></td>
                    <td className="px-5 py-4 text-white/55 text-xs">{new Date(a.submittedAt).toLocaleDateString()}</td>
                    ...
                    <td className="px-5 py-4 text-right text-xs space-x-2 whitespace-nowrap">
                      <Link to={`/asset/${a.slug}`} className="text-white/70 hover:text-white">View</Link>
                      ...
                      {a.status !== "Published" ? (
                        ...
                        <button onClick={() => setStatus(a.id, { status: "Approved" })} className="text-white/70 hover:text-white">Unpublish</button>
                      )}
