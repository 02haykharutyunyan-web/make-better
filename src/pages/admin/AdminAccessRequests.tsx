...
export default function AdminAccessRequests() {
  ...
  return (
    <AdminLayout eyebrow="Requests" title="Access requests">
      ...
      {loading && <div className="mb-6 card-premium p-4 text-sm text-white/55">Loading requests...</div>}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {requests.map(r => (
                <tr key={r.id} className="border-t border-white/5 align-top">
                  <td className="px-5 py-4">
                    ...
                    <div className="mt-1 text-xs text-white/45">{r.assets?.creators?.brand_name || ""}</div>
                  </td>
                  <td className="px-5 py-4">
                    <div className="text-white/80">{r.buyer_name}</div>
                    <div className="mt-1 text-xs text-white/45">{r.buyer_email}</div>
                  </td>
                  <td className="px-5 py-4 text-white/65">{r.buyer_phone || "-"}</td>
                  <td className="px-5 py-4 text-white/55">{new Date(r.created_at).toLocaleDateString()}</td>
                  <td className="px-5 py-4">
                    <select
                      value={r.status}
                      onChange={e => updateStatus(r.id, e.target.value as AccessRequestStatus)}
                      className="rounded-full border border-white/15 bg-black px-3 py-1.5 text-xs text-white/80"
                    >
              ...
              {!loading && requests.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-5 py-10 text-center text-white/55">No access requests yet.</td>
                </tr>
              )}
            </tbody>
          </table>
