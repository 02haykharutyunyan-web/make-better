          ...
          <table className="min-w-[900px] w-full text-sm">
            ...
            <tbody>
                ...
                <tr key={c.id} className="border-t border-white/5 align-top">
                  <td className="px-5 py-4">
                    <div className="flex items-start gap-3">
                      <div className="h-10 w-10 rounded-full border border-[#1E293B] bg-[#111827]/70 flex items-center justify-center font-medium">
                      ...
                      <div>
                        ...
                        <div className="mt-1 text-xs text-[#94A3B8]/80">{c.slug}</div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-5 py-4 text-[#94A3B8]">{new Date(c.joinedAt).toLocaleDateString()}</td>
                  <td className="px-5 py-4">
                    <span className={`rounded-full border px-3 py-1 text-xs ${c.featured ? "bg-amber-400/10 text-amber-300 border-amber-400/20" : "bg-[#111827]/80 text-[#94A3B8] border-[#1E293B]"}`}>
                      {c.featured ? "Featured" : "No"}
                    </span>
                  </td>
                  <td className="px-5 py-4 text-right text-xs space-x-3 whitespace-nowrap">
                    <Link to={`/creator/${c.slug}`} className="text-[#94A3B8] hover:text-white">View</Link>
              ...
              {!loading && creators.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-5 py-10 text-center text-[#94A3B8]">No creators found.</td>
                </tr>
              )}
            </tbody>
          </table>
