  ...
  return (
    <SiteLayout>
      ...
      <section className="container-mb mt-10">
        ...
        {!loading && visiblePosts.length === 0 && <div className="card-premium p-8 sm:p-10 text-center text-[#94A3B8]">No published posts yet.</div>}
          ...
          {visiblePosts.map(p => {
            ...
            return (
              <Link to={`/blog/${p.slug}`} key={p.slug} className="card-premium p-5 sm:p-7 group">
                ...
                <div className="mt-5 flex flex-wrap items-center justify-between gap-2 text-xs text-[#94A3B8]/80">
                  <span>{p.date}</span>
